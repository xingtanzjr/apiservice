...
func (c *ApiServiceController) Run(threadiness int, stopCh <-chan struct{}) error {
	...
	klog.Info("Start Api Service Controller")
...
func (c *ApiServiceController) fetchEventItemAndProcess() bool {
	...
	if err != nil {
		klog.Errorf("Error when processing api service event item %s", eventItem)
		utilruntime.HandleError(err)
		return true
	}
...
func (c *ApiServiceController) processOneEventItem(item EventItem) error {
	...
	// First, Sync the target api service across all cluster
	if err := c.syncApiServiceToAllCluster(apiService); err != nil {
		utilruntime.HandleError(err)
		klog.Error(err, "Error when syncing api service across cluster.")
	}
	return nil
}

func (c *ApiServiceController) syncApiServiceToAllCluster(target *apis.ApiService) error {
	for clusterId, tool := range c.clusterToolMap {
		...
		if err != nil {
			...
			if c.shouldUpdate(current, target) {
				...
				if err != nil {
					klog.Error(err, fmt.Sprintf("Failed to update api service[%s] on cluster[%s]", target.Name, clusterId))
				} else {
					klog.Infof("update api service[%s] on cluster [%s]", current.Name, clusterId)
				}
			}
		}
	}
...
func (c *ApiServiceController) shouldUpdate(current, target *apis.ApiService) bool {
